...
class KeyValueStoreListener extends Doctrine_Record_Listener
{
    ...
    public function preSave(Doctrine_Event $event)
    {
        if ( ! $this->_options['disabled']) {
            ...
            if ($invoker->hasMappedValue('_sfKeyValueStoreModified') && $invoker->_sfKeyValueStoreModified)
            {
              $invoker->$updatedName = serialize($invoker->_sfKeyValueStoreData);
              $invoker->_sfKeyValueStoreModified = false;
            }
        }
    }
}